...
import com.google.samples.apps.codechallenge.helper.JsonHelper;
import com.google.samples.apps.codechallenge.model.Category;
import com.google.samples.apps.codechallenge.model.JsonAttributes;
import com.google.samples.apps.codechallenge.model.Theme;
 ...
 */
public class CodeChallengeDatabaseHelper extends SQLiteOpenHelper{

    private static final String TAG = "CodeChallengeDatabaseHelper";
    private static final String DB_NAME = "todd";
    private static final String DB_SUFFIX = ".db";
    private static final int DB_VERSION = 1;
    private static List<Category> mCategories;
    private static CodeChallengeDatabaseHelper mInstance;
    private final Resources mResources;

    private CodeChallengeDatabaseHelper(Context context) {
        //prevents external instance creation
        super(context, DB_NAME + DB_SUFFIX, null, DB_VERSION);
        mResources = context.getResources();
    }

    private static CodeChallengeDatabaseHelper getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new CodeChallengeDatabaseHelper(context.getApplicationContext());
        }
        return mInstance;
    }
    ...
    private static List<Category> loadCategories(Context context) {
        Cursor data = CodeChallengeDatabaseHelper.getCategoryCursor(context);
        List<Category> tmpCategories = new ArrayList<>(data.getCount());
        final SQLiteDatabase readableDatabase = CodeChallengeDatabaseHelper.getReadableDatabase(context);
        do {
    ...
    /**
     * Resets the contents of todd's database to it's initial state.
     *
     * @param context The context this is running in.
     */
