...
package com.google.samples.apps.codechallenge.fragment;
...
import com.google.samples.apps.codechallenge.adapter.QuizAdapter;
import com.google.samples.apps.codechallenge.adapter.ScoreAdapter;
...
public class QuizFragment extends android.support.v4.app.Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mCategory = CodeChallengeDatabaseHelper.getCategoryWith(getActivity(), categoryId);
    ...
    public boolean showNextPage() {
        ...
        if (nextItem < count) {
            mQuizView.showNext();
            CodeChallengeDatabaseHelper.updateCategory(getActivity(), mCategory);
            return true;
        }
        markCategorySolved();
        return false;
    ...
    public void showSummary() {
        ...
        Category sCategory = mCategory;
        Integer res = 0;
        int arr_score[] = sCategory.getScores();
        for(int i=0;i<4;i++){
            if(arr_score[i] != 0){
                res = res + 25;
            }
...
//        System.out.print("senthilidea"+CodeChallengeDatabaseHelper.getScore(getActivity().getApplicationContext()));
//        Integer result1 = (Integer) CodeChallengeDatabaseHelper.getScore(getActivity().getApplicationContext());
        TextView result = (TextView) getView().findViewById(R.id.result_text);
        result.setText("You are "+res.toString()+"% likely to have Todd's Syndrome. ");
